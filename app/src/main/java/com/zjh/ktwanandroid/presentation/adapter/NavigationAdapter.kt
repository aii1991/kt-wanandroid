 ...
 * 2022/7/20
 */
class NavigationAdapter(data: MutableList<ArticleNavigation>) : BaseQuickAdapter<ArticleNavigation, BaseViewHolder>(R.layout.item_hierarchy,data) {
    var onItemClick:(data: Article, view: View, position: Int) -> Unit =
        { _: Article, _: View, _: Int -> }

    override fun convert(holder: BaseViewHolder, item: ArticleNavigation) {
        ...
        holder.getView<RecyclerView>(R.id.item_system_rv).run {
            ...
            adapter = NavigationChildAdapter(item.articles).apply {
                setOnItemClickListener { _, view, position ->
                    onItemClick(this.data[position], view, position)
                }
            }
        }
    }
