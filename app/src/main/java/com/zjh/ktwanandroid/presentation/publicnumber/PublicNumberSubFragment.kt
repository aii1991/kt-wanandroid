...
import com.zjh.ktwanandroid.databinding.FragmentPublicNumberSubBinding
...
@AndroidEntryPoint
class PublicNumberSubFragment : AbsArticleFragment<PublicNumberVM,FragmentPublicNumberSubBinding>(){
    companion object{
        fun newInstance(cId: Int): Fragment {
            return PublicNumberSubFragment().apply {
    ...
    override suspend fun observeUiState(coroutineScope: CoroutineScope) {
        coroutineScope.launch {
            mViewModel.mArticlePagingData.value.collectLatest { mArticleAdapter.submitData(it) }
        }
    }
